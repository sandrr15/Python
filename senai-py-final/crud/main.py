...
def update():
     ...
     if selecao:
         ...
         novo_imc = float(novo_peso) / (float(novo_altura)**2)
         if novo_nome and novo_idade:
            conn = conectar()
            c = conn.cursor()    
            c.execute('UPDATE usuarios SET nome = ? ,  idade = ? , peso = ? , altura = ?, imc = ?  WHERE id = ? ',(novo_nome,novo_idade,novo_peso,novo_altura,novo_imc,user_id,),)
            conn.commit()
            conn.close()  
...
btn_atualizar = tk.Button(janela, text='atualizar', command=update)
btn_atualizar.grid(row=25, column=0, padx=10, pady=10)



columns = ('ID','NOME', 'IDADE', 'ALTURA', 'PESO','IMC')
